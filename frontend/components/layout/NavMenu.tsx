...
export default function NavMenu({ items, isInternal = false }: NavMenuProps) {
        ...
        {items.map((item) => (
          <li key={item.id}>
            <Link
              ...
              className={`text-center text-[clamp(13px,1.4vw,16px)] xl:text-[18px] font-[900] leading-[60px] font-nav ${isInternal ? 'text-black' : 'text-white'}`}
            >
              {item.label}
            </Link>
          </li>
